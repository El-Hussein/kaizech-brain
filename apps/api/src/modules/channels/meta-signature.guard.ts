...
export class MetaSignatureGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // Resolve App Secret: tenant-level setting takes priority, fallback to global env
    ...
    if (!appSecret) {
      this.logger.error(
        'WHATSAPP_APP_SECRET is not configured — cannot validate Meta signature',
      );
      throw new ForbiddenException(
        'WhatsApp App Secret is not configured on this tenant',
      );
    }
    ...
    if (
      ...
      this.logger.warn(
        `Invalid Meta signature for tenant ${tenantId ?? 'unknown'}`,
      );
      throw new ForbiddenException(
        'Invalid X-Hub-Signature-256 — request did not come from Meta',
      );
    }
